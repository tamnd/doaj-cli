...
describes what users get. Pick a format with `-o`, or let doaj choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
doaj <command> -o table   # aligned columns for reading
doaj <command> -o jsonl   # one JSON object per line, for piping
doaj <command> -o json    # a single JSON array
doaj <command> -o csv     # spreadsheet friendly
doaj <command> -o tsv     # tab-separated
doaj <command> -o url     # just the URL column
doaj <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
doaj <command> --fields id,title,url
```
...
```bash
doaj <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
doaj <command>            # a table, because this is a terminal
doaj <command> | wc -l    # JSONL, because this is a pipe
```
